                        ...
                        if (currentPrinterCommand == null) {
                            ...
                            if (result == 0) {//打印机状态查询
                                ...
                                if ((buffer[0] & ESC_STATE_COVER_OPEN) > 0) {
                                    status += " "+AppApplication.getInstance().getString(R.string.str_printer_open_cover);
                                }
                                if ((buffer[0] & ESC_STATE_ERR_OCCURS) > 0) {
                                    status += " "+AppApplication.getInstance().getString(R.string.str_printer_error);
                                }
                                //Utils.toast(AppApplication.getInstance(), status);
                            }
                        }
